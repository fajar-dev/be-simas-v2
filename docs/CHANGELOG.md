...
### Added
- **`GET /inventory/export` — export Excel lengkap Item → Varian → Cabang → Kondisi**: satu baris per leaf (item, varian, cabang × kondisi) — item tanpa varian atau varian tanpa stok tetap dapat satu baris dengan kolom yang lebih dalam dikosongkan. Kolom: No, Image, Code, Name, Description, Category, Sub Category, Unit, Status, Variant Name, Variant Code, Variant Description, Branch, Condition, Quantity, plus kolom label dinamis (`labelColumns`). Mendukung filter & search yang sama seperti `GET /inventory`. Mengikuti pola kode `AssetUtilService.export` (header 2-baris berkelompok, baris selang-seling, border, autofilter, hyperlink gambar) — `InventoryUtilService` baru, permission `inventory:export` baru (di-seed). Swagger & `test/inventory.test.ts` (4 test baru, termasuk parsing buffer xlsx untuk verifikasi baris) diperbarui.
