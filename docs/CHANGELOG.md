...
### Changed
- **Lampiran pada Stock Out assign**: `POST /inventory-stock-out` menerima `attachmentIds` opsional (file yang sudah di-upload lewat `POST /attachment`). Karena satu assign multi-item bisa menghasilkan beberapa baris `InventoryStockOut` independen (bukan satu dokumen induk seperti Transfer/Stock In), baris pertama memiliki lampirannya (`AttachmentService.associate`), baris berikutnya mendapat **salinan metadata** yang menunjuk ke file yang sama (`AttachmentService.duplicate`, method baru) — jadi semua baris hasil satu aksi assign menampilkan lampiran yang sama. `GET/POST /inventory-stock-out` kini mengembalikan `attachments` per baris. Swagger & `test/inventory-stock-out.test.ts` diperbarui.
