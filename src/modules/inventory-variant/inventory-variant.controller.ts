...
export class InventoryVariantController {
    ...
    async index(c: Context) {
        ...
        return ApiResponse.success(c, await InventoryVariantSerializer.collection(variants))
    }

    async show(c: Context) {
        const variant = await this.service.getById(Number(c.req.param("id")))
        return ApiResponse.success(c, await InventoryVariantSerializer.single(variant))
    }

    async store(c: Context) {
        const data = c.req.valid("json" as never) as any
        const variant = await this.service.create(data)
        return ApiResponse.success(c, await InventoryVariantSerializer.single(variant), "Created", 201)
    }

    async update(c: Context) {
        const data = c.req.valid("json" as never) as any
        const variant = await this.service.update(Number(c.req.param("id")), data)
        return ApiResponse.success(c, await InventoryVariantSerializer.single(variant), "Updated")
    }
